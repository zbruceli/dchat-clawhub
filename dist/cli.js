 ...
 */
function getPassphrase(dataDir) {
...
function resolveAddress(dataDir, addressOrAlias) {
    if (/^[0-9a-f]{64}$/i.test(addressOrAlias))
...
function cmdAddress(dataDir, explicitSeed) {
    if (explicitSeed) {
...
async function cmdSend(dataDir, seed, target, message) {
...
async function cmdSendMedia(dataDir, seed, type, target, filePath) {
    const bot = new DchatBot({ seed, dataDir });
    console.log("Connecting...");
    await bot.start();
    await new Promise((r) => setTimeout(r, 3000));
...
function cmdHistory(dataDir, seed, peerAddress, limit) {
    const bot = new DchatBot({ seed, dataDir });
...
async function cmdListen(dataDir, seed) {
    const bot = new DchatBot({ seed, dataDir });
...
async function cmdInteractive(dataDir, seed) {
    const bot = new DchatBot({ seed, dataDir });
    const contactDb = new MessageDb(dataDir);
    const shutdown = async () => {
        console.log("\nShutting down...");
