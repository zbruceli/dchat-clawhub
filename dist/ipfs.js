...
export class IpfsService {
    ...
    async download(ipfsHash, preferredIp) {
    ...
    isPrivateOrReservedIp(ip) {
        ...
        if (v4Parts.length === 4) {
            ...
            const [a, b] = octets;
            if (a === 10 || a === 127 || a === 0)
                return true; // 10.0.0.0/8, loopback, 0.0.0.0
            if (a === 172 && b >= 16 && b <= 31)
        ...
        if (v4Mapped)
            return this.isPrivateOrReservedIp(v4Mapped[1]);
        return false;
    }
    /**
    ...
    async orderGateways(preferredIp) {
        if (!preferredIp)
    ...
    uploadToGateway(gw, data, fileName) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = client.request({ hostname: gw.host, port: gw.port, path: "/api/v0/add", method: "POST", headers }, (res) => {
                let body = "";
    ...
    downloadFromGateway(gw, ipfsHash) {
        const headers = {};
        if (gw.authHeader)
            headers["Authorization"] = gw.authHeader;
        return new Promise((resolve, reject) => {
