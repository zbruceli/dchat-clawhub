...
export class DchatBot extends EventEmitter {
    ...
    constructor(config = {}) {
        ...
        this.nkn.on("message", (src, payload) => {
            this.handleIncoming(src, payload).catch((err) => console.error("[dchat-bot] Error handling message:", err));
        });
        this.nkn.on("statusChange", (status) => {
    ...
    async sendAudio(to, filePath, durationSeconds) {
    ...
    // ── Internal ─────────────────────────────────────────────
    buildMessage(contentType, content, options) {
        return {
            id: crypto.randomUUID(),
            contentType,
            content,
    ...
    async sendAndStore(to, msg, localFilePath) {
        const myAddress = this.nkn.getAddress();
    ...
    sendAndStoreNoReply(to, msg, localFilePath) {
        const myAddress = this.nkn.getAddress();
        this.storeOutbound(myAddress, to, msg, "sent", localFilePath);
        this.nkn.sendNoReply(to, JSON.stringify(msg));
    }
    storeOutbound(from, to, msg, status, localFilePath) {
    ...
    async handleIncoming(src, payload) {
        let msgData;
        try {
        ...
        if (!msgData.id || !msgData.contentType)
            return;
        // Handle receipts
        ...
        if (!DISPLAYABLE_TYPES.has(msgData.contentType))
            return;
        const myAddress = this.nkn.getAddress();
        ...
        if (!isSelfMessage)
            this.db.upsertPeer(src);
        // Send delivery receipt (skip self — avoids receipt loop)
        if (!isSelfMessage)
            this.sendReceipt(src, msgData.id);
        // Build event payload
        const incoming = {
            id: msgData.id,
            from: src,
        ...
        if (this.config.autoDownloadMedia && msgData.options) {
            try {
                const localPath = await this.downloadMediaFromOptions(msgData.id, msgData.contentType, msgData.content ?? "", msgData.options);
                if (localPath) {
    ...
    async downloadMediaFromOptions(messageId, contentType, content, options) {
        // Inline audio (base64 data-URI)
        if (contentType === "audio" && content.startsWith("![audio]")) {
            return this.media.saveInlineAudio(messageId, content, options.fileExt ?? "aac");
        }
        // IPFS media
        if (options.ipfsHash && options.ipfsEncryptKeyBytes) {
            return this.media.downloadMedia(options.ipfsHash, options.ipfsEncryptKeyBytes, options.ipfsEncryptNonceSize ?? 12, options.fileExt ?? "bin", options.ipfsIp);
        }
        return null;
    }
    startBurnScheduler() {
